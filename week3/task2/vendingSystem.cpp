...
const char* VendingSystem::getAddressById(const char* idToSearch){
    for(int i=0;i<size;++i){
...
void VendingSystem::getCurrentProductsById(const char* idToSearch){
    for(int i=0;i<size;++i){
        if(machines[i].checkID(idToSearch)){
...
void VendingSystem::sellProductsById(const char* idToSearch,int saltyToSell,int sweetToSell,int drinksToSell){
    for(int i=0;i<size;++i){
        if(machines[i].checkID(idToSearch)){
...
void VendingSystem::addProductsById(const char* idToSearch,int saltyToAdd,int sweetToAdd,int drinksToAdd){
    for(int i=0;i<size;++i){
        if(machines[i].checkID(idToSearch)){
